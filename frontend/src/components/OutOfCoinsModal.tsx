  ...
  const onAdComplete = async () => {
    ...
    try {
      ...
      if (data.granted_coins > 0) {
        // Small confirmation; the modal already updates in place.
        ...
        <View style={styles.root}>
          <Animated.View
            style={[
              ...
              {
                opacity: entrance,
                transform: [{ scale: entrance.interpolate({ inputRange: [0, 1], outputRange: [0.96, 1] }) }],
              },
            ]}
          >
            ...
            <LinearGradient colors={[theme.colors.primary, theme.colors.accent]} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.actionCard}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.actionSub}>{pairProgressLabel || 'Each 30s ad is half the coins for one match.'}</Text>
                {progress && (
